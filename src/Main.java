...
public class Main {
    public static void main(String[] args) {
        try {
            ...
            Optional<Integer> resultYear = students.stream().peek(System.out::println).map(Student::getBooks).flatMap(List::stream).sorted().distinct().filter(book -> book.getYear() > 2000).limit(3).map(Book::getYear).findFirst();
